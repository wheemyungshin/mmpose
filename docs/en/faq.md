...
- **What is `yolov7_predictions.json`? Can I train pose models without it?**

  "yolov7_predictions.json" contains the "detected" human bounding boxes for COCO validation set, which are generated by FasterRCNN.
  ...
  the generalizability of models, by setting `use_gt_bbox=False` and `bbox_file='yolov7_predictions.json'`.
